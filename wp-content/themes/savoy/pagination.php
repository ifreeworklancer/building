<?php
    ...
    if ( $infinite_load ) {
        ...
        $next_posts_link = get_next_posts_link( esc_html__( 'Загрузить еще', 'nm-framework' ) );
        $previous_posts_link = null;
    } else {
        $container_class = 'static';
        $next_posts_link = get_next_posts_link( esc_html__( 'Старые посты', 'nm-framework' ) );
        $previous_posts_link = get_previous_posts_link( esc_html__( 'Новые посты', 'nm-framework' ) );
    }
